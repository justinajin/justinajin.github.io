 ...
 */
document.userbookFrm.onsubmit = function () {
...
if (
  !regExpTest(
    /^[\w]{4,}@[\w]+(\.[\w]+){1,3}$/, //abc.com / aaa.abc.com 가능!
    email,
    "이메일 형식에 어긋납니다."
  )
)
...
function isEqualPwd() {// 비밀번호 체크 blur
...
class User {
  constructor (userId, userName, email, tel1, tel2, tel3) {
    this.userId = userId;
  ...
  toString () {
    return `User(userId = ${this.userId}, userName = ${this.userName}, email = ${this.email},
      tel1 = ${this.tel1}, tel2 = ${this.tel2}, tel3 = ${this.tel3})`;
  }
}
...
const saveUserbook = () => {
  ...
  const frm = document.userbookFrm;
  ...
  const user = new User(userId.value, userName.value, email.value, tel1.value, tel2.value, tel3.value);
  console.log(user);
                                //여기에 key값을 준다
...
const loadUserbook = () => {
  const tbody = document.querySelector("table#tbl-userbook tbody");
  const userList = JSON.parse(localStorage.getItem('userList'));

  // guestbook -> tr -> tbody
  tbody.innerHTML = userList.reduce((html, {userId, userName, email, tel1, tel2, tel3}, index) => {
    const tr = `
    <tr>
      <th>${index +1}</th>
      <th>${userId}</th>
